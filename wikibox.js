...
var wikibox = {
    'proxy' : 'localhost/gndProxy/gnd.php?query=',
...
//                            , othernames = ( typeof obj.synSearch !== "undefined") ? '<p class="WikiBoxSynonyms"><strong>Andere Namen</strong>'+ obj.altname.length > 30 ? obj.altname.substring(0, 30) : obj.altname +'<a class="synSearch" href="'+ baseSysUrl + encodeURIComponent( obj.synSearch )+'"> suche</a></p>' 
    ...
    jQuery(wikibox.loc).on("click", '.wikibox-gndlink', function(e) {
        ...
        if (jQuery('#wikibox-' + id).length >= 1) { jQuery('#wikibox-' + id).show();
            ...
            console.log('query data: '+ wikibox.proxy + id + wikibox.services + lang.split("_")[0]);
            jQuery.ajax(wikibox.proxy + id + wikibox.services + lang.split("_")[0] + '&jsoncallback=?', {
                type: 'GET',
                dataType: 'json',
...
//                        resp[prop] = resp[prop].replace(/<>/g," ").trim(); //replace angle brackets with space
                        resp[prop] = resp[prop].replace(/</g,"(").replace(/>/g,")").trim(); //mask homonyms with brackets 
                        len++;
